...
FourWheelRobot::FourWheelRobot(std::string name, sf::Vector2f positions, 
    ...
    // wheelbase = mainbody width(80.f) +  half wheel length(10.f/2 = 5.f)
    m_wheelBase = 85.f;
...
void FourWheelRobot::draw(sf::RenderTarget& target, sf::RenderStates states) const {
...
std::unique_ptr<sf::CircleShape>  FourWheelRobot::createCircleComponent(float radius, 
...
std::unique_ptr<RoundedRectangleShape> FourWheelRobot::createRoundedComponent(
